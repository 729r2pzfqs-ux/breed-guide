#!/usr/bin/env python3
"""
Apply Japanese translations to breed pages.
Fixes remaining English phrases in Best For, Not Ideal For, and temperament tags.
"""

import re
from pathlib import Path

from ja_translations import BEST_FOR, NOT_IDEAL, TEMPERAMENT_TAGS

JA_BREEDS_DIR = Path(__file__).parent / "ja" / "breeds"
...
def apply_translations(html):
    """Apply all translations to Japanese HTML."""
    updated = html
...
def main():
    breeds = sorted([f.stem for f in JA_BREEDS_DIR.glob("*.html")])
    print(f"Total Japanese breeds: {len(breeds)}")

    translated = 0
    for i, breed in enumerate(breeds):
        ja_path = JA_BREEDS_DIR / f"{breed}.html"
        ja_html = ja_path.read_text()

        updated_html = apply_translations(ja_html)

        if updated_html != ja_html:
            ja_path.write_text(updated_html)
            translated += 1
            print(f"  Updated: {breed}")
