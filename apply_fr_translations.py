#!/usr/bin/env python3
"""
Apply all French translations to breed pages.
"""

import re
from pathlib import Path
from fr_translations import BEST_FOR, NOT_IDEAL, TEMPERAMENT_TAGS
...
def apply_translation(html_content):
    """Apply translations to French HTML."""
    updated = html_content
    
    # Replace Best For items (emerald bullets)
    ...
    # Replace Not Ideal items (rose bullets)
...
def main():
    breeds = sorted([f for f in FR_BREEDS_DIR.glob("*.html")])
    print(f"Total French breeds: {len(breeds)}")
