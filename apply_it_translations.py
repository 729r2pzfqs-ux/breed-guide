#!/usr/bin/env python3
"""
Apply all Italian translations to breed pages.
"""

import re
from pathlib import Path
from it_translations import BEST_FOR, NOT_IDEAL, TEMPERAMENT_TAGS
...
def apply_translation(html_content):
    """Apply translations to Italian HTML."""
    updated = html_content
    
    # Replace Best For items (emerald-500 dot pattern)
    ...
    # Replace Not Ideal items (rose-500 dot pattern)
    ...
    # Replace temperament tags (sky-100 to blue-100 gradient pattern)
    ...
    # Also handle the emerald-50 variant for temperament tags
    def replace_emerald_tags(match):
        original = match.group(1).strip()
        translated = translate_text(original, TEMPERAMENT_TAGS)
        return f'bg-emerald-50 text-emerald-700 border border-emerald-200 px-4 py-2 rounded-full text-sm font-medium">{translated}</span>'
    
    pattern = r'bg-emerald-50 text-emerald-700 border border-emerald-200 px-4 py-2 rounded-full text-sm font-medium">([^<]+)</span>'
    updated = re.sub(pattern, replace_emerald_tags, updated)
    
    return updated

def main():
    breeds = sorted([f for f in IT_BREEDS_DIR.glob("*.html") if f.stem != "index"])
